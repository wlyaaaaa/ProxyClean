...
function Get-PCClientInventory {
    ...
    @($groups.Values|Sort-Object label|ForEach-Object{$_.ports=@($_.ports|Sort-Object -Unique);$_})
}
...
function Get-PCClientClosePreview {
    ...
    if($client.requires_admin -and -not(Test-PCAdministrator)){return [pscustomobject]@{status='client_needs_admin';key=$client.key;label=$client.label}}
    ...
    $plan=[pscustomobject]@{schema='proxyclean.client-close.v1';sid=$snapshot.sid;key=$client.key;label=$client.label;members=$identities;ports=@($client.ports);services=$services;observedUtc=$snapshot.observedUtc}
...
function Request-PCClientWindowClose {
    param($Identity)
    if(Test-PCClientProcess $Identity){$p=Get-Process -Id $Identity.pid -ErrorAction Stop;if($p.MainWindowHandle -ne [IntPtr]::Zero){[void]$p.CloseMainWindow()}}
}
...
function Invoke-PCClientClose {
    ...
    param([Parameter(Mandatory)]$Plan,[switch]$Force,[scriptblock]$Progress,[ValidateRange(0,10)][int]$WaitSeconds=4)
    ...
    try{
        ...
        if($Force){
            ...
            foreach($member in @($Plan.members)){if(Test-PCClientProcess $member){Stop-Process -Id $member.pid -Force -ErrorAction Stop}}
        }
        ...
        if($remaining.Count -or $sameClient.Count -or $occupied.Count){
            return [pscustomobject]@{status='client_still_running';key=$Plan.key;label=$Plan.label;force_attempted=[bool]$Force;previous_plan=$Plan;settings_changed=$false;message='客户端未完全退出，或端口被重新占用。尚未清理代理设置。'}
        }
        ...
        [pscustomobject]@{status='client_closed';label=$Plan.label;settings=$settings;connectivity=$probe;remaining=$limits.ToArray();all_applications_direct='not_proven'}
