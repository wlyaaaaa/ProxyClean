...
只运行一个已识别客户端时，点击一次“关闭代理，恢复普通上网”即可开始。多个客户端同时运行时，先选定一个。需要管理员权限时只保留 Windows 的 UAC 授权；新窗口核验原 Windows 用户与同一批进程后继续本次关闭，不再追加程序内确认。普通打开、旧入口和只读预览不会自动关闭代理。

同一次明确关闭请求先尝试正常退出；超时后自动结束原选择中仍存活且身份未变的残留进程，先控制界面、再核心和辅助进程。不根据名称或旧端口追杀新进程，不关闭其他客户端，不改服务启动方式，不重启网卡。只有原本运行的已识别启动辅助服务会在结束后恢复，包含关闭失败和设置清理异常的路径；独立代理核心服务不会被当成启动辅助服务重新启动。恢复失败或核心意外重新出现时，不报告关闭成功。
...
确认退出后刷新 Windows 域名缓存，减少已失效的隧道 Fake-IP 缓存残留；缓存刷新失败单独报告。不会自动更换 DNS 服务器、改写独立 DNS 服务配置或重置网卡。基础网页探测连续出现解析错误时，明确提示检查本机 DNS 及可脱离代理的上游，不能把重复清理代理当成 DNS 修复。关闭本机代理也不保证海外网站可以直接访问。
...
基础网页测试先请求百度，再按需回退微软测试站点；每次请求均有超时。只有主动诊断、修复、关闭或显式测试才会请求，打开首页不会请求。请求跳过显式代理不等于绕过 TUN，结果不会据此证明所有流量直连。维护窗口的公网出口比较须另行确认后请求 ipify。管理员授权保留所选操作和原 Windows 用户；一次点击关闭按上述规则继续，网卡等其他高影响操作保留原有确认。
...
修改入口支持 `-WhatIf`；关闭端口、网卡重置、IPv6 变更和撤销还要求确认。只有显式给出 `-ExtraProcessName`，关闭端口入口才追加所列客户端进程。这个命令行端口入口不会自动追加整套客户端进程；旧端口已关闭也不会导致其他端口的客户端被结束。首页的客户端关闭流程将一次点击作为该客户端正常退出及有界残留处理的明确请求，自动完成内部预检。
...
测试隔离真实代理、路由和进程效果，覆盖混合配置、地址族、回滚、并发变化、DPAPI、Git 原子写入、断线网卡、进程身份、IPv6、脱敏、辅助服务恢复、退出竞争、DNS 失败分类和实际 GUI 按钮衔接。零发现、发现错误和未运行不能算通过。`ControlCenter.ps1 -SelfTest` 只验证窗口构造；`-SmokeTest` 会实际打开窗口、执行只读检查并自动关闭，返回工作线程与中文进度的结果，不证明网络恢复。
...
本次一次点击关闭、启动辅助服务与 DNS 验证的实测范围见 [2026-09-25 验收记录](docs/verification-2026-09-25.md)。
